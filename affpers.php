      ...
      <div class="col-sm-9 col-md-6 col-lg-8 col-xl-10">
        <nav aria-label="breadcrumb">
          <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item" aria-current="page"><a href="#">afficher_chercheur</a></li>
          </ol>
        </nav>
        <div class="Nompersonne">Afficher  Chercheur</div>
        <table class="table">
          <thead>
            <tr><th>Nom</th><th>image</th><th>Email</th><th>Fonction</th><th>Projet</th><th>Equipe</th><th>Laboratoire</th></tr>
          </thead>
          <tbody>
            <?php
             include('connectbdd.php');
             $reponse=$bd->query('SELECT * FROM Chercheur');
             while(!empty($donnee=$reponse->fetch()))
                  {
            ?>
            <tr class="info">
              <td><?php echo $donnee['nom_chercheur']; ?></td>
              <td><img src="img/<?php echo $donnee['profile_chercheur']; ?>" style="width:40px;"></td>
              <td><?php echo $donnee['email_chercheur']; ?></td>
              <td><?php echo $donnee['fonction_chercheur']; ?></td>
              <?php
               include('connectbdd.php');
               $reponse2=$bd->prepare('SELECT nom_projet FROM Projet where id_projet=?');
               $reponse2->execute(array($donnee["id_projetchercheur"]));
              ...
              <?php
               include('connectbdd.php');
               $reponse3=$bd->prepare('SELECT nom_equipe FROM Equipe where id_equipe=?');
               $reponse3->execute(array($donnee["id_equipechercheur"]));
               $donnee3=$reponse3->fetch();

              ?>
              <td><?php echo $donnee3['nom_equipe']; ?></td>
              <?php
               include('connectbdd.php');
               $reponse4=$bd->prepare('SELECT nom_labo FROM Labo where id_labo=?');
               $reponse4->execute(array($donnee["id_labochercheur"]));
               $donnee4=$reponse4->fetch();

              ?>
              <td><?php echo $donnee4['nom_labo']; ?></td>
            </tr>
            <?php
              }$reponse->closeCursor(); ?>
          </tbody>
