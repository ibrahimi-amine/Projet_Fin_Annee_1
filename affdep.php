      ...
      <div class="col-sm-9 col-md-6 col-lg-8 col-xl-10">
        <nav aria-label="breadcrumb">
          <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item" aria-current="page"><a href="#">afficher depense</a></li>
          </ol>
        </nav>
        <div class="Nompersonne">Afficher  Depense</div>
        <table class="table">
          <thead>
            <tr><th>N_Programme d'emploi</th><th>Code Rubrique</th><th>Code Depense</th><th>Description</th><th>Montant</th></tr>
          </thead>
          <tbody>
            <?php
             include('connectbdd.php');
             $reponse=$bd->query('SELECT * FROM Depence');
            ...
            <tr class="info">
              <?php
               include('connectbdd.php');
               $reponse2=$bd->prepare('SELECT N_pe,code_rubrique FROM Depencer where N_depense=?');
               $reponse2->execute(array($donnee["N_depense"]));
               $donnee2=$reponse2->fetch();

              ?>
              <td><?php echo $donnee2['N_pe']; ?></td>
              <td><?php echo $donnee2['code_rubrique']; ?></td>
              <td><?php echo $donnee['N_depense']; ?></td>
