      ...
      <div class="col-sm-9 col-md-6 col-lg-8 col-xl-10">
        <nav aria-label="breadcrumb">
          <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item" aria-current="page"><a href="#">afficher programme d'emploi</a></li>
          </ol>
        </nav>
        <div class="Nompersonne">Afficher  Programme d'emploi</div>
        <?php
         include('connectbdd.php');
         $reponse=$bd->query('SELECT * FROM Programme_emploi');
         while(!empty($donnee=$reponse->fetch()))
              {
        ?>
        <div class="alert alert-primary" role="alert">Information Programme D'emploi :</div>
        <div class="row">
          <div class="col-4"><div class="text text-primary">Id Programme Emploi : </div><?php echo $donnee['N_pe']; ?></div>
          <div class="col-4"><div class="text text-primary">Date affectation  Programme Emploi :</div> <?php echo $donnee['date_pe']; ?></div>
          <div class="col-4"><div class="text text-primary">Budget Programme Emploi : </div><?php echo $donnee['budget_pe']; ?></div>
        </div>
        <div class="row">
          <div class="col-6"><div class="text text-primary">Id Budget Projet  : </div><?php echo $donnee['id_budgetprojet']; ?></div>
          <div class="col-6"><div class="text text-primary">Id Budget Fonctionnelle : </div><?php echo $donnee['id_budgetfonct']; ?></div>
        </div>
        <div class="alert alert-primary" role="alert">Credit Rubrique :</div>
        <table class="table">
          <thead>
            <tr><th>Code Rubrique</th><th>Designation</th><th>Credit ouvert</th><th>Credit Disponible</th></tr>
          </thead>
          <tbody>
            <?php
             include('connectbdd.php');
             $reponse2=$bd->Prepare('SELECT * FROM Credit where N_pe=?');
             $reponse2->execute(array($donnee['N_pe']));
             while(!empty($donnee2=$reponse2->fetch()))
                  {
            ?>
            <tr class="info">
              <td><?php echo $donnee2['code_rubrique']; ?></td>
              <?php
               include('connectbdd.php');
               $reponse3=$bd->prepare('SELECT Designation FROM Rubrique where code=?');
               $reponse3->execute(array($donnee2["code_rubrique"]));
               $donnee3=$reponse3->fetch();

              ?>
              <td><?php echo $donnee3['Designation']; ?></td>
              <td><?php echo $donnee2['credit_ouvert']; ?></td>
              <td><?php echo $donnee2['credit_disponible']; ?></td>
            </tr>
            <?php
