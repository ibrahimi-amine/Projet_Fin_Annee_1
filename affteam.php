  ...
  <div class="col-sm-9 col-md-6 col-lg-8 col-xl-10">
    <nav aria-label="breadcrumb">
      <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item" aria-current="page"><a href="#">afficher_equipe</a></li>
      </ol>
    </nav>
    <div class="Nompersonne">Afficher  equipe</div>
    <table class="table">
      <thead>
        <tr><th>Nom equipe</th><th>responsable equipe</th><th>Date creation</th><th>Description</th></tr>
      </thead>
      <tbody>
        <?php
         include('connectbdd.php');
         $reponse=$bd->query('SELECT * FROM Equipe');
         while(!empty($donnee=$reponse->fetch()))
              {
        ?>
        <tr class="info">
          <td><?php echo $donnee['nom_equipe']; ?></td>
          <?php
          $reponse2=$bd->prepare('SELECT * FROM Chercheur WHERE id_chercheur=?');
          $reponse2->execute(array($donnee['id_responsableequipe']));
          $donnee2=$reponse2->fetch();
          ?>
          <td><?php echo $donnee2['nom_chercheur']; ?></td>
          <td><?php echo $donnee['date_creationequipe']; ?></td>
          <td><?php echo $donnee['description_equipe']; ?></td>
